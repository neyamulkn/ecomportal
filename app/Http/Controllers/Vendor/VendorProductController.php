   ...
   public function index(Request $request, $status='')
    {
        ...
        $data['all_products'] = Product::where('vendor_id', $vendor_id)->count();
        $data['stockout_products'] = Product::where('vendor_id', $vendor_id)->where('stock', '<=', 0)->count();
        $data['image_missing'] = Product::where('vendor_id', $vendor_id)->where('feature_image', null)->count();
        $data['active_products'] = Product::where('vendor_id', $vendor_id)->where('status', 'active')->count();
        $data['inactive_products'] = Product::where('vendor_id', $vendor_id)->where('status', 'pending')->count();
    ...
    public function store(Request $request)
    {
        ...
        // Insert product
        $data = new Product();
        $data->vendor_id = ($request->vendor_id ? $request->vendor_id : $this->vendor_id());
        ...
        $data->ship_region_id = 4;
        $data->other_region_cost = 130;
        $data->shipping_time = '1-3 days';
        ...
        if($store) {
            ...
            if ($request->attribute) {
                ...
                foreach ($request->attribute as $attribute_id => $attr_value) {
                    ...
                    $feature->save();
                    if($request->attributeValue && array_key_exists($attribute_id, $request->attributeValue)) {

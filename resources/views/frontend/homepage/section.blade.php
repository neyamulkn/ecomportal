...
$products = App\Models\Product::with('offer_discount.offer:id')->whereIn('id', explode(',', $section->product_id))->where('status', 'active')->selectRaw('id,title,selling_price,discount,discount_type, slug, feature_image')->orderBy('id', 'desc')->take($section->item_number)->get();
?>
@if(count($products)>0)
